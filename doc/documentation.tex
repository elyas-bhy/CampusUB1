...
Il existe plusieurs outils utiles à l'implémentation d'une solution à ce problème, notamment JFlex/CUP qui, d'après un livre d'Android~\cite{ProgAndroid}. Ces outils ressortent pour nous, car leur utilisation nous est déjà familière. Ces outils cumulés avec les informations extraites d'un livre d'analyse textuelle~\cite{analyse textuelle}\\
...
\chapter{Tests et Validation}
  ...
  \section{Test 06: Annuaire}
    ...
    \underline{Résultats attendus}:
    \begin{itemize}
    \item Si l'application est connectée à Internet: on arrive sur l'écran de la~[figure~\ref{fig:07}], qui affiche tous les contacts des établissements auxquels on est abonné.
