...
\chapter{Tests et Validation}
  ...
  \section{Test 01: Choix de l'établissement}
    ...
    \item Sélctionnez un des éléments dans le menu.
    \end{enumerate}

    Résultat attendu: Vous serez redirigé vers le menu correspondant à l'établissement choisi. Exemple pour le choix de Bordeaux 1:~[Fig.~\ref{fig:02}].
  ...
  \section{Test 02: Maintient du choix de l'établissement}
    ...
    Résultat attendu: L'application aura sauvegardé votre choix d'établissement, et vous redirigera à la page d'accueil de cet établissement. Exemple pour le choix LaBRI:~[Fig.~\ref{fig:09}]
  ...
  \section{Evénements (Bordeaux1 et LaBRI)}  
    ...
    \subsection{Ajout dans l'agenda depuis la vue détaillé de l'événement} %WTF
      Depuis l'écran des détails de l'événement, appuyez un moment au milieu de la page et un menu apparaîtra. Celui-ci sera plus ou moins rempli en fonction des détails fournis dans le texte. Vous pouvez rentrer les informations manquantes (par exemple changer de titre qui sera le titre de l'événement, ...) puis valider. Vous pouvez, à partir de ce moment, aller regarder dans l'agenda du smartphone, l'événement devrait être ajouté aux horaires choisis.
